...
from http_server.server import IterativeServer
...
def parse_sys_args():
    ...
    parser.add_argument("-w", "--workers", dest="workers", default=10)
...
def main(args):
    ...
    try:
        for i in xrange(args.workers):
            server = IterativeServer(args.host, args.port, os.path.realpath(args.document_root), logger)
